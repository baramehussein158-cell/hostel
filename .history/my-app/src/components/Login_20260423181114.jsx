...
const Login = ({
  ...
  const [resetStatusData, setResetStatusData] = useState({
    email: '',
    regNumber: '',
    campus: '',
    gender: '',
  ...
  const handlePasswordResetConfirmSubmit = async (event) => {
    ...
    if (!canUseResetForm) {
      setFeedback({ type: 'error', text: 'Wait for admin approval before entering a new password.' });
      return;
    }
  ...
  const handleCheckResetStatus = () => {
    ...
    if (request.status === 'pending') {
      ...
      // Auto-populate the reset code form with the student's details and code
      setResetCodeData({
        email: resetStatusData.email,
        regNumber: resetStatusData.regNumber,
        campus: resetStatusData.campus,
        gender: resetStatusData.gender,
      ...
      setFeedback({
        type: 'success',
        text: `Your request was approved! The reset code has been filled in below. Set your new password.`,
      });
          ...
          {mode === 'reset' && (
            <div className="reset-mode">
              ...
              <form onSubmit={handlePasswordResetConfirmSubmit} className="reset-card">
                ...
                <button type="submit" className="login-btn" disabled={isSubmitting || isSyncing || !canUseResetForm}>
                  {isSubmitting ? 'Updating...' : canUseResetForm ? 'Set New Password' : 'Waiting for Approval'}
                </button>
              </form>
            </div>
          )}
